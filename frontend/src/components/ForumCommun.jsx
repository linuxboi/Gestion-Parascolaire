...
const ImageGrid = () => {
  ...
  useEffect(() => {
    const fetchPublications = async () => {
      try {
        const response = await axios.get('https://gestion-parascolaire.onrender.com/api/publications', {
          headers: {
  ...
  const handleReact = async (publicationId, type) => {
    ...
    try {
      if (existingReactionIndex !== -1) {
        ...
        await axios.delete(`https://gestion-parascolaire.onrender.com/api/publications/${publicationId}/react`, {
          headers: {
            Authorization: `Bearer ${token}`, // Pass the token in headers
          },
        ...
        await axios.post(`https://gestion-parascolaire.onrender.com/api/publications/${publicationId}/react`, { type }, {
          headers: {
            Authorization: `Bearer ${token}`, // Pass the token in headers
          },
        });
  ...
  const handleAddComment = async () => {
    if (newComment && selectedPublication) {
      try {
        await axios.post(`https://gestion-parascolaire.onrender.com/api/publications/${selectedPublication._id}/comments`, { texte: newComment }, {
          headers: {
            Authorization: `Bearer ${token}`, // Pass the token in headers
          },
        });
